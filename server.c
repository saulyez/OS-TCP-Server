...
void delete_rules(struct node **head, const char *ip,const char *port, int
client_port);
...
//TODO
NULL) return;
if (*connections_head == NULL || (*connections_head)->matched_connections ==
...
while (temp != NULL && strcmp(temp->ip, ip) == 0 && strcmp(temp->port, port) ==
*head = temp->next;
...
// A
int argcount = sscanf(request, "%s %s %s %s", command, new_ip, new_port,
extra);
if (argcount > 3) {
print_send("Illegal request\n", *newsockfd);
...
if (pthread_create(&server_thread, NULL, processRequest, (void
*)new_sockfd) == 0) {
